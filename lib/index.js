...
/**
 * 
 * @param {string|object} usernameOrOptions - username או אובייקט עם apiKey
 * @param {string} password - סיסמה (אופציונלי אם משתמשים ב-apiKey)
 * @param {import("axios").AxiosRequestConfig} config 
 * @param {string} ym_server 
 */
function Yemot_api(usernameOrOptions, password, config = {}, ym_server = "ym") {
	...
	// תמיכה במפתח API או username/password
	if (typeof usernameOrOptions === 'object' && usernameOrOptions.apiKey) {
		apiKey = usernameOrOptions.apiKey;
		token = apiKey; // מפתח API הוא בעצם הטוקן
		// אם יש גם config באובייקט
		if (usernameOrOptions.config) {
			config = { ...usernameOrOptions.config, ...config };
		}
		// אם יש גם ym_server באובייקט
		if (usernameOrOptions.ym_server) {
			ym_server = usernameOrOptions.ym_server;
		}
	} else {
		// שימוש מסורתי עם username/password (נשמר לעתיד)
	...
	// אימות מפתח API באתחול
	if (apiKey) {
		// אימות באמצעות GetSession
		(async () => {
			try {
				...
				console.error("שגיאה באימות מפתח API:", error.message);
			}
		})();
	}
	...
	this.getIvrDir = (path, options = {}) => {
		...
		// הוספת פרמטרים אופציונליים אם הם סופקו
	...
	/**
	 * פונקציה פנימית לביצוע קריאה ישירה ללא אתחול אוטומטי
	 */
	...
	async function exec(method, parameters = {}, options = {}) {
		...
		if (!is_connect && method !== "Login" && method !== "GetSession") {
			if (apiKey) {
				// אם משתמשים במפתח API, נבצע GetSession לאימות
				try {
					...
					throw new Error("מפתח API לא תקין: " + error.message);
				}
			} else {
				// אם משתמשים ב-username/password, נבצע login
				await login();
			}
		}
		...
		async function session_is_expired() {
			// אם משתמשים במפתח API קבוע, המפתח עצמו לא תקין
			if (apiKey) {
				throw new Error("מפתח API לא תקין או פג תוקף");
			}
			
			// אחרת, ננסה להתחבר מחדש עם username/password
			await login();
