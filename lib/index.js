...
const { textToSpeech, getFirstTwoWords } = require("./tts");
...
function Yemot_api(username, password, config = {}, ym_server = "ym") {
	...
	/**
	 * Convert text to speech using Google TTS API and upload to Yemot
	 * @param {string} text - The text to convert to speech
	 * @param {string} googleApiKey - Google Cloud API key
	 * @param {string} path - The path to upload the file to in Yemot
	 * @param {Object} options - Additional options
	 * @param {string} options.voice - The voice to use (default: 'he-IL-Wavenet-D')
	 * @param {number} options.speed - The speaking rate (default: 1.0)
	 * @param {string} options.filename - Custom filename (default: first two words of text)
	 * @param {boolean} options.convertAudio - Whether to convert audio format (default: true)
	 * @returns {Promise} - Promise with the upload result
	 */
	this.text_to_speech = async (text, googleApiKey, path, options = {}) => {
		if (!text || !googleApiKey || !path) {
			throw new Error("Missing required parameters: text, googleApiKey, and path are required");
		}

		const voice = options.voice || 'he-IL-Wavenet-D';
		const speed = options.speed || 1.0;
		
		// Convert text to speech using Google TTS API
		const audioBuffer = await textToSpeech(text, googleApiKey, voice, speed);
		
		// Generate filename based on first two words of text if not provided
		const filename = options.filename || getFirstTwoWords(text);
		...
		// Upload the file to Yemot
		return this.upload_file(path, file, true);
	};
