...
### דרך 1: שימוש עם מספר מערכת ומפתח API (ברירת מחדל - מומלץ)
```js
...
**פרמטרים:**
- `systemNumber` - מספר המערכת (פרמטר ראשון) - **ניתן להעביר `null` אם לא רוצים אימות מספר מערכת**
- `apiKey` - מפתח API קבוע (פרמטר שני)
- `config` - אובייקט הגדרות נוספות ל-axios (פרמטר שלישי - אופציונלי)
- `ym_server` - שרת ימות (פרמטר רביעי - אופציונלי, ברירת מחדל: "ym")

**כברירת מחדל**, הפרמטר השני מטופל כמפתח API. המערכת תאמת שהמפתח שייך למספר המערכת שהועבר.

**שימוש ללא אימות מספר מערכת:**
```js
const y = new yemot_api(null, "YOUR_API_KEY_HERE");
```
כאשר מעבירים `null` כפרמטר ראשון, המערכת תשתמש במפתח API בלבד ללא אימות התאמה למספר מערכת.

### דרך 2: שימוש עם אובייקט אפשרויות
```js
const yemot_api = require("yemot-api");
const y = new yemot_api({ apiKey: "YOUR_API_KEY_HERE", systemNumber: "0773137770" });
```

**אובייקט אפשרויות:**
- `apiKey` - מפתח API קבוע (חובה)
- `systemNumber` - מספר המערכת לאימות (אופציונלי) - אם מצוין, המערכת תאמת שהמפתח שייך למספר זה
- `config` - אובייקט הגדרות נוספות ל-axios (אופציונלי)
- `ym_server` - שרת ימות (אופציונלי, ברירת מחדל: "ym")

### דרך 3: שימוש עם אימות דו-שלבי (2FA) - שם משתמש וסיסמה
```js
const yemot_api = require("yemot-api");
// אפשרות 1: עם פרמטר בוליאני
const y = new yemot_api("0773137770", "1234", true);

// אפשרות 2: עם אובייקט config
const y = new yemot_api("0773137770", "1234", { use2FA: true });
```

**פרמטרים:**
- `username` - שם משתמש ביומות (פרמטר ראשון)
- `password` - סיסמה ביומות (פרמטר שני)
- `configOrUse2FA` - הפרמטר השלישי יכול להיות:
  - `true` - מציין שזו התחברות עם אימות דו-שלבי
  - אובייקט עם `use2FA: true` - מציין שזו התחברות עם אימות דו-שלבי
  - אובייקט config רגיל - במקרה זה הפרמטר השני ייחשב למפתח API (ברירת מחדל)
- `ym_server` - שרת ימות (פרמטר רביעי - אופציונלי, ברירת מחדל: "ym")

**הערה:** אימות דו-שלבי יתווסף בעתיד. כרגע השימוש הוא רק עם מפתח API.
...
// אתחול עם אימות מספר מערכת
const y = new YemotApi({ 
    apiKey: "YOUR_API_KEY_HERE",
    systemNumber: "0773137770"  // המערכת תוודא שה-API שייך למספר זה
});
```

### אבטחה - אימות מספר מערכת
ניתן לשרשר את מספר המערכת לאימות שהמפתח API שייך למערכת הנכונה. זה מונע שימוש בטעות במפתח של מערכת אחרת:

```js
const y = new YemotApi({ 
    apiKey: "YOUR_API_KEY_HERE",
    systemNumber: "0773137770"  // מספר המערכת המצופה
});

// בעת ההתחברות, המערכת תקרא ל-GetSession ותאמת שהשדה username
// מתאים למספר המערכת שהועבר. אם לא - תזרק שגיאה:
// "System number mismatch: expected 0773137770, but got 07xxxxxxxx"
```

**תרחיש שימוש:** כאשר עובדים עם מספר מערכות יומות (למשל בסביבות staging/production), השרשור מונע שימוש בטעות במפתח API של מערכת אחת במערכת אחרת.
